...
public class ModifyMaintenanceActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (intent != null) {
			maintenance = (Maintenance) intent
					.getSerializableExtra("maintenance");
			if (maintenance != null) {
				maintenance_name = (EditText) findViewById(R.id.maintenance_name);
				maintenance_address = (EditText) findViewById(R.id.maintenance_address);
				maintenance_appliance = (EditText) findViewById(R.id.maintenance_appliance);
				spinner = (Spinner) findViewById(R.id.spinner);
				maintenance_name.setText(maintenance.getMaintenance_name());
				maintenance_address.setText(maintenance
						.getMaintenance_address());
				maintenance_appliance.setText(maintenance
						.getMaintenance_appliance());
				stringList.add("未接取");
				stringList.add("已取消");
				spinner.setAdapter(new ReusableAdapter<String>(stringList,
				...
				Toast.makeText(ModifyMaintenanceActivity.this, "请先选择一项在进行修改",
	...
	public void doClick(View view) {
		switch (view.getId()) {
		case R.id.submit:
			final String maintenanceName = maintenance_name.getText()
					.toString().trim();
			final String maintenanceAddress = maintenance_address.getText()
					.toString().trim();
			final String maintenanceAppliance = maintenance_appliance.getText()
					.toString().trim();
			if (maintenanceName == null || "".equals(maintenanceName)) {
				...
				Toast.makeText(ModifyMaintenanceActivity.this, "请对维修申请进行简单描述",
						Toast.LENGTH_SHORT).show();
			} else if (maintenanceAddress == null
					|| "".equals(maintenanceAddress)) {
				maintenance_address.setFocusable(true);
				maintenance_address.setFocusableInTouchMode(true);
				maintenance_address.requestFocus();
				Toast.makeText(ModifyMaintenanceActivity.this,
						"请输入此次维修申请的详细地址信息", Toast.LENGTH_SHORT).show();
			} else if (maintenanceAppliance == null
					|| "".equals(maintenanceAppliance)) {
				...
				Toast.makeText(ModifyMaintenanceActivity.this,
						"请输入维修申请的其他家电信息和详情", Toast.LENGTH_SHORT).show();
			} else {
				new Thread() {
					@Override
					public void run() {
						...
						try {
							...
							if (conn != null) {
								...
								ps = conn
										.prepareStatement("UPDATE maintenance SET maintenance_name = ?, maintenance_address = ?, maintenance_appliance = ?, maintenance_status = ? WHERE maintenance_id = "
												+ maintenance
														.getMaintenance_id());
								...
								ps.setInt(4,
										maintenance.getMaintenance_status());
								int exeNums = ps.executeUpdate();
								LogUtil.logV("ModifyMaintenanceActivity",
										"更新条数：" + exeNums);
								if (exeNums == 1) {
									runOnUiThread(new Runnable() {
										...
										public void run() {
											runOnUiThread(new Runnable() {
												...
												public void run() {
													Toast.makeText(
															ModifyMaintenanceActivity.this,
															"修改成功！",
															Toast.LENGTH_LONG)
															.show();
												}
