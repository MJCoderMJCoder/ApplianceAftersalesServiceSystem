...
public class AddMaintenanceActivity extends Activity {
	...
	private final int MAINTENANCE_STATUS = 0; // 0-未接取【用户】/可接取【维修工】
												// 1-已接取【用户】/未完成【维修工】 3-已取消
												// 4-已完成
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		maintenance_user = (Integer) SPUtil.get(getApplicationContext(), "user_id", 1);
	...
	public void doClick(View view) {
		...
		case R.id.submit:
			...
			if (maintenanceName == null || "".equals(maintenanceName)) {
				...
				Toast.makeText(AddMaintenanceActivity.this, "请对维修申请进行简单描述",
				...
				Toast.makeText(AddMaintenanceActivity.this, "请输入此次维修申请的详细地址信息",
						Toast.LENGTH_SHORT).show();
				...
				new Thread() {
					...
					public void run() {
						...
						try {
							...
							if (conn != null) {
								ps = conn
										.prepareStatement("INSERT INTO maintenance (maintenance_name, maintenance_address, maintenance_appliance, maintenance_user, maintenance_status) VALUES (?, ?, ?, ?, ?)");
								...
								LogUtil.logV("AddMaintenanceActivity", "插入条数："
										+ exeNums);
								if (exeNums == 1) {
