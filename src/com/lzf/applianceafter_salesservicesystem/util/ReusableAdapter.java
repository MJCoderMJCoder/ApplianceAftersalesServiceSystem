...
public abstract class ReusableAdapter<T> extends BaseAdapter {
	...
	/**
	 * 定义一个抽象方法，完成ViewHolder与相关数据集的绑定； 创建新的BaseAdapter的时候，只需实现这个方法就好！
	 * 
	 * @param holder
	 *            视图持有者：用于对ListView中的item的View进行子视图渲染
	 * @param obj
	 *            该适配器要处理的一系列原始数据中的某项数据的对象信息
	 * @see ViewHolder
	 */
	...
	public static class ViewHolder {
		 ...
		 * @return 与item绑定后的ViewHolder（视图持有者：用于对ListView中的item的View进行子视图渲染）
		 */
		public ViewHolder setOnTouchListener(int id,
		 ...
		 *            视图控件的资源id
		 * @param visible
		 *            可见状态的具体值：VISIBLE、INVISIBLE、GONE
		 * @return 与item绑定后的ViewHolder（视图持有者：用于对ListView中的item的View进行子视图渲染）
		 */
		 ...
		 *            视图控件的资源id
		 * @param obj
		 *            设置 ImageView 的网络图片的URL
		 * @param context
		 *            环境/上下文
		 * @return 与item绑定后的ViewHolder（视图持有者：用于对ListView中的item的View进行子视图渲染）
		 */
		// public ViewHolder setImageByGlide(int id, String obj, Context
		// context) {
		// View view = getView(id);
		// if (view instanceof ImageView) {
		// Glide.with(context)
