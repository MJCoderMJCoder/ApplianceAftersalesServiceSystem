 ...
 * must implement the {@link WorkFieldFragment.OnFragmentInteractionListener}
 * interface to handle interaction events. Use the
 * {@link WorkFieldFragment#newInstance} factory method to create an instance of
 * this fragment.
 */
@SuppressLint("NewApi")
public class WorkFieldFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View view = inflater.inflate(R.layout.fragment_work_field, container,
				false);
		context = getActivity();
		...
		reusableAdapter = new ReusableAdapter<Maintenance>(maintenanceList,
				R.layout.item_maintenance_work) {
			@Override
			public void bindView(ViewHolder holder, final Maintenance obj) {
				...
				case 1:
					...
					holder.setOnClickListener(R.id.done,
									...
									new Thread() {
										...
										public void run() {
											...
											try {
												...
												if (conn != null) {
													ps = conn.prepareStatement("UPDATE maintenance SET maintenance_status = ? WHERE maintenance_id = "
													...
													if (exeNums == 1) {
														ps = conn
																.prepareStatement("SELECT * FROM maintenance where maintenance_maintainer = "
																		+ SPUtil.get(
																				context,
																				"maintainer_id",
																				0));
									...
									new Thread() {
										...
										public void run() {
											...
											try {
												...
												conn = DBUtil.getConnection();
												if (conn != null) {
													ps = conn.prepareStatement("UPDATE maintenance SET maintenance_status = ?, maintenance_maintainer = null WHERE maintenance_id = "
															+ obj.getMaintenance_id());
													ps.setInt(1, 0);
													int exeNums = ps
															.executeUpdate();
				...
				holder.setOnClickListener(R.id.detail,
						new View.OnClickListener() {
							@Override
							public void onClick(View v) {
								Intent intent = new Intent(context, MaintainerDetailActivity.class);
								intent.putExtra("maintenance", obj);
			...
			public void run() {
				...
				try {
					...
					if (conn != null) {
						ps = conn
								.prepareStatement("SELECT * FROM maintenance where maintenance_maintainer = "
										+ SPUtil.get(context, "maintainer_id",
												0));
						rs = ps.executeQuery();
						while (rs.next()) {
