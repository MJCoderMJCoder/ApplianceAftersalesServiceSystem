 ...
 * must implement the {@link PartFragment.OnFragmentInteractionListener}
 * interface to handle interaction events. Use the
 * {@link PartFragment#newInstance} factory method to create an instance of this
 * fragment.
 */
@SuppressLint("NewApi")
public class PartFragment extends Fragment {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// applianceList = (List<Appliance>)
		// getArguments().getSerializable(APPLIANCE_LIST);
		// }
	}
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		Button add_part = (Button) view.findViewById(R.id.add_part);
		if ("18334706003".equals(SPUtil.get(context, "maintainer_phone", ""))
				|| "超级维修员".equals(SPUtil.get(context, "maintainer_name", ""))) {
			add_part.setVisibility(View.VISIBLE);
			add_part.setOnClickListener(new View.OnClickListener() {
				@Override
				public void onClick(View v) {
					startActivity(new Intent(context, AddPartActivity.class));
		...
		ListView partList = (ListView) view.findViewById(R.id.part_list);
		reusableAdapter = new ReusableAdapter<Appliance>(applianceList,
				R.layout.item_part) {
			@Override
			public void bindView(ViewHolder holder, Appliance obj) {
				holder.setText(R.id.appliance_name,
						"家电名称：" + obj.getAppliance_name());
				holder.setText(R.id.appliance_model,
						"家电型号：" + obj.getAppliance_model());
				holder.setText(R.id.appliance_part,
						"零件名称：" + obj.getAppliance_part());
				holder.setText(R.id.appliance_price,
						"零件价格：" + obj.getAppliance_price() + "");
			...
			public void run() {
				...
				try {
					...
					if (conn != null) {
						...
						while (rs.next()) {
							applianceList.add(new Appliance(rs
									.getInt("appliance_id"), rs
									.getString("appliance_name"), rs
									.getString("appliance_model"), rs
									.getString("appliance_part"), rs
									.getFloat("appliance_price"), rs
									.getString("appliance_arg")));
						}
						LogUtil.logV("MaintainerHomeActivity",
								applianceList.toString());
